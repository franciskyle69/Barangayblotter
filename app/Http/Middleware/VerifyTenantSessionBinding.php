...
class VerifyTenantSessionBinding
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // than "mismatched". If the request has an unambiguous tenant
        // context, we bind the session on the fly. This avoids redirect
        // loops where the user is authenticated but the binding key is
        // missing (common after deployments).
        if ($sessionAuthTenantId === false) {
            $resolvedTenant = app()->bound('current_tenant') ? app('current_tenant') : null;

            if (!$user->is_super_admin && $resolvedTenant) {
                $request->session()->put('auth_tenant_id', $resolvedTenant->id);
                if (!$request->session()->has('current_tenant_id')) {
                    $request->session()->put('current_tenant_id', $resolvedTenant->id);
                }
    ...
    private function forceLogout(Request $request, string $reason): Response
    {
        ...
        if ($request->header('X-Inertia')) {
            // Inertia requests must never receive plain JSON. Force a client-side visit.
        ...
        // Path-based to keep redirects on the current host (tenant domain vs central).
        return redirect('/login')->with('error', 'Your session is no longer valid. Please sign in again.');
    }
}
