...
export default function TenantSignup({ plans = [] }) {
    ...
    const selectedPlan = plans.find(
        (plan) => String(plan.id) === String(data.requested_plan_id),
    );
    const selectedPlanPrice = Number(selectedPlan?.price_monthly ?? 0);
    const requiresStripeCheckout = Number.isFinite(selectedPlanPrice)
        ? selectedPlanPrice > 0
        : false;
    ...
    }, [data.custom_domain, data.subdomain, isLocalDomainFlow, setData]);
                ...
                >
                    ...
                    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                        ...
                        <p className="mb-3 text-xs text-slate-600">
                            ...
                            assigned as Barangay Admin. A temporary password
                            is generated and sent after the request is
                            approved.
                        </p>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div>
                    ...
                    <div>
                        ...
                        {requiresStripeCheckout && (
                            <p className="mt-2 text-xs text-slate-600">
                                Paid plan selected. On submit, you will be
                                redirected to Stripe Checkout to complete
                                payment before your request is submitted.
                            </p>
                        )}
                    </div>
