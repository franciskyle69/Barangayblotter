...
export default function TenantUsers({ tenant, users, roles }) {
  ...
  const handleCreate = (e) => {
    ...
    postCreate(`/super/tenants/${tenant.id}/users/create-account`, {
      preserveScroll: true,
      onSuccess: () => resetCreate('name', 'email', 'phone', 'password', 'password_confirmation'),
    });
  };
      ...
      <form onSubmit={handleCreate} className="mb-6 rounded-lg bg-white p-6 shadow">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div>
            ...
            <input type="password" value={createData.password} onChange={(e) => setCreateData('password', e.target.value)} className={inputClass} required />
            {createErrors.password && <p className={errorClass}>{createErrors.password}</p>}
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Confirm Password *</label>
            <input type="password" value={createData.password_confirmation} onChange={(e) => setCreateData('password_confirmation', e.target.value)} className={inputClass} required />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Role *</label>
