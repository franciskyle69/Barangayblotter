                ...
                {canEdit && (
                    <div className="flex gap-2">
                        ...
                        {canScheduleMediation && (
                            <Link
                                href={`/incidents/${incident.id}/mediations/create`}
                ...
                <div className="rounded-lg bg-white p-6 shadow">
                    <dl className="space-y-3">
                        ...
                        <div>
                            ...
                            <dd>
                                <span
                                    className={`rounded px-2 py-0.5 text-sm ${STATUS_CLASS[incident.status] || "bg-slate-100 text-slate-800"}`}
                                >
                    ...
                    {incident.mediations?.length > 0 && (
                        <div className="rounded-lg bg-white p-6 shadow">
                            ...
                            {incident.mediations.map((med) => (
                                <div
                                    key={med.id}
                                    className="mb-3 border-b border-slate-100 pb-3 last:border-0"
                                >
                                    <p className="font-medium">
                                        Scheduled:{" "}
                                        {formatDate(med.scheduled_at)}
                                    </p>
                                    <p className="text-sm text-slate-600">
                                        Mediator: {med.mediator?.name}
                                    </p>
                                    <p className="text-sm">
                                        Status: {med.status}
                                    </p>
                                    {med.settlement_terms && (
