...
export default function Register() {
  const { errors, logo_url, registrationRoleOptions, current_tenant } = usePage().props;
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#f9fafb] px-4 font-sans antialiased">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-lg border border-slate-200/80 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-slate-800">Register</h2>
          <form onSubmit={(e) => { e.preventDefault(); post('/register'); }} className="space-y-4">
            <div>
            ...
            <div>
              <label htmlFor="requested_role" className="mb-1 block text-sm font-medium text-slate-700">Role</label>
              <select
                id="requested_role"
              ...
              <p className="mt-1 text-xs text-slate-500">Selecting admin/staff roles requires super admin approval after registration.</p>
            </div>
            <div>
          ...
          <p className="mt-4 text-center text-sm text-slate-600">
            Already have an account? <a href="/login" className="font-medium text-devias-primary hover:underline">Sign in</a>
          </p>
        </div>
      </div>
    </div>
